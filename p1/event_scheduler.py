...
class EventScheduler:
    ...
    def schedule_event(self, event):
        ...
        try:
            ...
            logger.info(
                f"Scheduled event {event_id} ({symbol} {event_type}) for {event_time} (delay: {delay:.0f}s)"
            )
    ...
    def trigger_event(self, event):
        ...
        try:
            ...
            logger.info(
                f"🎯 EVENT_FIRE id={event_id} symbol={symbol} type={event_type}"
            )
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                logger.info(
                    f"🔄 DEBUG: Loop #{loop_count} - Cargando eventos próximos..."
                )
                ...
                logger.info(
                    f"🔧 DEBUG: Esperando {self.check_interval} segundos hasta próxima verificación..."
                )
...
def start_event_scheduler(db_path="trades.db", check_interval=300):
    ...
    scheduler_thread = threading.Thread(
        target=scheduler.run, daemon=False
    )  # CAMBIADO A False
