...
class SLTPManager:
    ...
    def ensure_relation(self, entry_price: Decimal, sl_pct: float, tp_pct: float) -> tuple:
        ...
        if not is_valid and self.retry_count < self.max_retries:
            logger.warning(f"Invalid SL/TP relation - nudging prices (attempt {self.retry_count + 1})")
            self.retry_count += 1
        ...
        logger.info(f"✅ SL/TP relation validated: SL={sl_price:.2f}, TP={tp_price:.2f}")
