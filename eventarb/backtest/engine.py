...
def run_backtest(actions: List[PlannedAction], price_data: pd.DataFrame) -> List[Dict]:
    ...
    for action in actions:
        ...
        entry_price = Decimal(str(price_data['close'].iloc[0] if len(price_data) > 0 else 10000))
...
def simulate_trade_outcome(entry_price: Decimal, tp_price: Decimal, 
                          sl_price: Decimal, price_data: pd.DataFrame) -> Dict:
    """Simulate trade outcome based on price data"""
    if price_data.empty:
        # Default simulation if no data
        return {'exit_price': entry_price * Decimal('1.02'), 'pnl_pct': 2.0, 'win': True}
    ...
    for _, row in price_data.head(24).iterrows():
        ...
        if current_price >= tp_price:
            return {'exit_price': tp_price, 'pnl_pct': float((tp_price - entry_price) / entry_price * 100), 'win': True}
        elif current_price <= sl_price:
            return {'exit_price': sl_price, 'pnl_pct': float((sl_price - entry_price) / entry_price * 100), 'win': False}
