...
class DeepSeekClient:
    ...
    def _make_request(self, prompt: str) -> Dict[str, Any]:
        ...
        data = {
            "model": "deepseek-chat",
            "messages": [
                {"role": "system", "content": "Eres un experto en Python y debugging. Proporciona soluciones claras y específicas."},
    ...
    def _extract_files(self, content: str) -> list:
        ...
        lines = content.split('\n')
        for line in lines:
            if any(keyword in line.lower() for keyword in ['.py', '.sh', '.yaml', '.yml']):
