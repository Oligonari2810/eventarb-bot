...
def start_event_scheduler():
    ...
    try:
        ...
        scheduler = EventScheduler(
            db_path="trades.db", check_interval=60
        ...
        logger.info(
            f"✅ Event Scheduler iniciado directamente - {len(scheduler.active_timers)} timers activos"
        )
    ...
    def run_bot_cycle(self):
        ...
        try:
            ...
            if not os.path.exists(python_executable):
                ...
                logger.warning(
                    "⚠️  Usando Python del sistema (entorno virtual no encontrado)"
                )
            else:
                logger.info(
                    f"✅ Usando Python del entorno virtual: {python_executable}"
                )
