...
def finalize_trade(db_path: str, trade_id: int, exit_price: float):
    ...
    try:
        ...
        cur.execute("SELECT side, entry_price, quantity FROM trades WHERE id=?", (trade_id,))
        trade = cur.fetchone()
...
def get_trade_summary(db_path: str = None):
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*), COALESCE(SUM(pnl_usd), 0) FROM trades WHERE date(closed_at) = ?", (today,))
        result = cur.fetchone()
...
def close_all_simulated_trades(db_path: str = None):
    ...
    try:
        ...
        cur.execute("SELECT id, side, entry_price, quantity, tp_price FROM trades WHERE simulated = 1 AND closed_at IS NULL")
        trades = cur.fetchall()
