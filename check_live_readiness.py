...
def main():
    ...
    required_vars = [
        "EVENT_SHEET_ID", "TRADES_SHEET_ID", 
        "EVENT_SHEET_RANGE", "TRADES_SHEET_RANGE",
    ...
    try:
        ...
        valid_events = 0
        for event in events:
            if hasattr(event, 'id') and hasattr(event, 'kind') and hasattr(event, 'symbols'):
                valid_events += 1
        ...
        for event in events[:5]:  # Mostrar solo los primeros 5
            if hasattr(event, 'id') and hasattr(event, 'kind'):
                symbols_str = ', '.join(event.symbols) if hasattr(event, 'symbols') else 'N/A'
