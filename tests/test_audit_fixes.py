...
from p1.daily_counter import (
    get_daily_count, calculate_daily_pnl, increment_daily_count,
    update_daily_loss, get_bot_state
)
...
class TestAuditFixes(unittest.TestCase):
    ...
    def test_event_fires_idempotency(self):
        ...
        cursor = conn.execute("SELECT COUNT(*) FROM event_fires WHERE event_id = ?", (event_id,))
        count = cursor.fetchone()[0]
        
        self.assertEqual(count, 1, "Debería haber solo una entrada por (event_id, window_sec)")
        ...
        self.assertEqual(count, 2, "Debería permitir múltiples entradas con diferentes window_sec")
